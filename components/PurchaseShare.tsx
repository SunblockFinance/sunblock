...
import { alpha, Backdrop, Button, CircularProgress, colors } from "@mui/material";
...
import {
  CONTRACT_ADDRESS_SUNBLOCK, TOKEN_ADDRESS_DEMOERC20
...
export const PurchaseShares: FC = () => {
  ...
  async function purchaseShare(amount: number) {
    ...
    try {
      ...
      const response:TransactionResponse = await sunblockContract.buyShares(amount)
    ...
    sunblockContract.on(
      'SharesIssued',
      async (_address:string, amount) => {
        console.log(`Shares issued to ${_address}. Looking for ${signerAddress}`);
        if (signerAddress !== _address ) {return}
 ...
 <Button
    ...
    >
      ...
      <Backdrop
        sx={{ backgroundColor: alpha(colors.deepPurple[900], 0.9), color: '#fff', zIndex: (theme) => theme.zIndex.drawer + 1 }}
        open={open}
      ...
      >
        ...
        <Button target='_blank' rel='noopener' variant='outlined' href={`https://mumbai.polygonscan.com/tx/${activeTx?.hash}`}>See on polyscan</Button>
        </Stack>
      </Backdrop>
    </Stack>
  )
    ...
    const getBalance = async () => {
      try {
        ...
        erc20.on("Transfer", (from, to, amount, event) => {
          console.log(`Incoming transfer from ${from} to ${to} with value ${amount}`)
          console.log(`We are looking for ${signerAddress} to ${CONTRACT_ADDRESS_SUNBLOCK}`);

          if (from == signerAddress && to == CONTRACT_ADDRESS_SUNBLOCK){
            console.log("We got a match")
            async () => {
              console.log('Updating balance after transfer')
